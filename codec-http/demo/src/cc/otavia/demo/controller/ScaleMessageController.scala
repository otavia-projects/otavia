...
object ScaleMessageController {
    ...
    private val paramSerde: ParameterSerde[ScaleLength] = new ParameterSerde[ScaleLength] {

        override def serialize(value: ScaleLength, out: Buffer): Unit = {
            serializeParamsStart(out)
            serializeParam("length", value.length.toString, out)
            serializeParamsEnd(out)
        }

        override protected def construct(params: mutable.HashMap[String, String]): ScaleLength =
            ScaleLength(params("length").toInt)
    ...
    val messageRequestSerde: HttpRequestFactory[ScaleLength, ScaleMessage, ScaleMessage] =
        new HttpRequestFactory[ScaleLength, ScaleMessage, ScaleMessage](Some(messageSerde), Some(paramSerde)) {
            override def createHttpRequest(): HttpRequest[ScaleLength, ScaleMessage, ScaleMessage] =
