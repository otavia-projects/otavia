...
class ActorSystemConfigBuilder {
    ...
    private[config] def build(): ActorSystemConfig = {
        ...
        val rawPoolSize = _actorThreadPoolSize
            ...
            .orElse(
              SysPropResolver
                  .floatOpt("cc.otavia.actor.worker.ratio")
        ...
        val poolSize =
            if (rawPoolSize < 1) {
                Report.report(
                  s"cc.otavia.actor.worker.size can't set $rawPoolSize, it must >= 1, set $defaultCpus default"
                )
                defaultCpus
            } else rawPoolSize
    ...
    private[config] def build(): BufferConfig = {
        ...
        val pageSize =
            if (enablePageSizes.contains(kb)) kb * 1024
                ...
                Report.report(
                  s"cc.otavia.buffer.page.size is set to $kb, but only support ${enablePageSizes
                          .mkString("[", ", ", "]")}, set to default ${4 * 1024}",
                  "Buffer"
                )
                4 * 1024
            }
...
class SchedulerConfigBuilder {
    ...
    private[config] def build(): SchedulerConfig = {
        ...
        val minBudgetMicros =
            if (rawMinBudget < 1) {
                Report.report(
                  s"cc.otavia.actor.min.budget.microsecond can't set $rawMinBudget, it must >= 1, set 500 default"
                )
                500
            } else rawMinBudget
        ...
        val stealAggression =
            if (rawStealAggression <= stealFloor) {
                Report.report(
                  s"cc.otavia.core.steal.aggression can't set $rawStealAggression, it must > stealFloor($stealFloor), set 128 default"
                )
                128
...
class ReactorConfigBuilder {
    ...
    private[config] def build(): ReactorConfig = {
        ...
        val rawNioSize = _nioWorkerSize
            ...
            .orElse(
              SysPropResolver
                  .floatOpt("cc.otavia.nio.worker.ratio")
                  .map(r => (r * defaultCpus).toInt)
            )
            .getOrElse(defaultCpus)
        val nioSize =
            if (rawNioSize < 1) {
                Report.report(
                  s"cc.otavia.nio.worker.size can't set $rawNioSize, it must >= 1, set $defaultCpus default"
                )
                defaultCpus
            } else rawNioSize
    ...
    private[config] def build(): ChannelConfig = {
        ...
        val (writeWaterMarkLow, writeWaterMarkHigh) =
            if (rawLow < 1 || rawHigh < 1 || rawLow >= rawHigh) {
                Report.report(
                  s"cc.otavia.channel.writeWaterMark[$rawLow, $rawHigh] invalid, must satisfy 1 <= low < high, set [32768, 65536] default"
                )
                (32768, 65536)
            } else (rawLow, rawHigh)
    ...
    private[config] def build(): TimerConfig = {
        ...
        val tickDurationMs =
            if (rawTickDuration < 1) {
                Report.report(
                  s"cc.otavia.timer.tickDurationMs can't set $rawTickDuration, it must >= 1, set 100 default"
                )
                100L
        ...
        val ticksPerWheel =
            if (rawTicksPerWheel < 1) {
                Report.report(
                  s"cc.otavia.timer.ticksPerWheel can't set $rawTicksPerWheel, it must >= 1, set 512 default"
                )
                512
...
class PriorityConfigBuilder {
    ...
    private[config] def build(): PriorityConfig = {
        ...
        val highPriorityReplySize =
            if (rawReplySize < 1) {
                Report.report(
                  s"cc.otavia.priority.highPriorityReplySize can't set $rawReplySize, it must >= 1, set 2 default"
                )
                2
            } else rawReplySize
        ...
        val highPriorityEventSize =
            if (rawEventSize < 1) {
                Report.report(
                  s"cc.otavia.priority.highPriorityEventSize can't set $rawEventSize, it must >= 1, set 4 default"
                )
                4
            } else rawEventSize
